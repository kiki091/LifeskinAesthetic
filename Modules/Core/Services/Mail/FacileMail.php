...
use Modules\Core\Models\MailJobs as MailModel;
use Modules\Core\Jobs\SendEmail;
...
class FacileMail
{
    ...
    /*
    TO USE:
        use App\AyanaMail as MailService;
    ...
    */
    ...
    const DEFAULT_SENDER = "contact@qeon.co.id";
        ...
        Mail::send(['html' => 'mail/' . $view], ['data' => $data['body']], function ($message) use ($data) {
            try {
                if(!array_key_exists('from', $data))
                    $data['from'] = self::DEFAULT_SENDER;

                if(!array_key_exists('from_alias', $data))
                    $data['from_alias'] = 'QDH';

                $message->subject($data['subject'])
                    ->to($data['to'])
                    ->from($data['from'] , $data['from_alias'])
                    ->replyTo($data['from']);
    ...
    public function sendQueueMailWithLog($to = "", $from = 'default', $subject = "", $blade = "welcome", $data = array(), $bcc = "")
    {
        try{
            $data['to'] = $to;
            $data['from'] = $from == 'default' ? self::DEFAULT_SENDER : $from;
            $data['from_alias'] = isset($data['default_name']) ? $data['default_name'] : 'QDH Mail';
            $data['subject'] = $subject;
            $data['body']   = $data;
            $data['bcc']     = $bcc;
