  ...
  $.widget('ui.weekCalendar', (function() {
    ...
    return {
      options: {
        ...
        selectedTime: {start: 9, mid: 13, end: 17},
      ...
      _resizeCalendar: function() {
        ...
        if (options && $.isFunction(options.height)) {
          ...
          this.element.find('.wc-scrollable-grid').height('243px');
...
//              calendarHeaderHtml+=    "</div>";
      ...
      _renderCalendarBody: function($calendarContainer) {
        ...
        $calendarContainer.find('.wc-full-height-column').height(options.timeslotHeight * 1);
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _renderCalendarBodyEvents: function($calendarTableTbody) {
        ...
        if (options.showSelectedTime){
          for (var i = 0; i < 3; i++) {
            ...
            renderRow += '<div class=\"wc-time-header-cell\">' + self._selected24HourForIndex(i) + '</div>';
            renderRow += '</div>';
          } 
        } else {
        ...
        var triggerElement = '<div class="add-slot"></div>';

        var options = this.options,
            timeStart = options.selectedTime.start,
            timeMid = options.selectedTime.mid,
            timeEnd = options.selectedTime.end;
        for (var i = 1; i <= options.daysToShow; i++) {
          if (!showAsSeparatedUser) {
            renderRow += '<td class=\"' + _columnBaseClass + ' wc-day-column-first wc-day-column-last day-' + i + '\">';
            renderRow += '<div class=\"wc-full-height-column row-1 wc-day-column-inner day-' + i + ' column-row-' + i + '\" data-time=\"' + timeStart + '\"></div>';
            renderRow += '<div class=\"wc-full-height-column row-2 wc-day-column-inner-2 day-' + i + ' column-row-' + i + '\" data-time=\"' + timeMid + '\"></div>';
            renderRow += '<div class=\"wc-full-height-column row-3 wc-day-column-inner-3 day-' + i + ' column-row-' + i + '\" data-time=\"' + timeEnd + '\"></div>';
            renderRow += '</td>';

            $('.wc-full-height-column').append(triggerElement);
          }
          else {
            var uLength = options.users.length;
            var columnclass;
      ...
      _setupEventCreationForWeekDay: function($weekDay) {
          var self = this;
          var options = this.options;
          var clickTrigger = $weekDay.find('.sotr-add')
      ...
      _updateDayColumnHeader: function($weekDayColumns) {
          ...
          if (this.options.title) {
            ...
            title = title.replace('%start%', self._formatDate(start, 'F Y'));
      ...
      _renderEvent: function(calEvent, $weekDay) {
          ...
          if (options.showSelectedTime){
            ...
            var calEvents = calEvent.class; 

            if (calEvent.id) {
              self.element.find('.wc-cal-event').each(function() {
                  if ($(this).data('calEvent').class === "sotr-single") {
                    $(this).addClass('sotr-single');
                  }
                  else if ($(this).data('calEvent').class === "sotr-twin") {
                    $(this).addClass('sotr-twin');
                  }
                  else if ($(this).data('calEvent').class === "sotr-warning") {
                    $(this).addClass('sotr-warning');
                  }
                  else if ($(this).data('calEvent').class === "sotr-close") {
                    $(this).addClass('sotr-close');
                  }
                  else if ($(this).data('calEvent').class === "sotr-add") {
                    $(this).addClass('sotr-add');
                  }
                  else{
                    return;
                  }
      ...
      _positionEvent: function($weekDay, $calEvent) {
        var options = this.options;
        if (options.showSelectedTime) {
          var calEvents = $calEvent.data('calEvent').class;
        } else {
          var calEvent = $calEvent.data('calEvent');
          var pxPerMillis = $weekDay.height() / options.millisToDisplay;
          var firstHourDisplayed = options.businessHours.limitDisplay ? options.businessHours.start : 0;
          var firstSelectHourDisplayed = options.selectedTime.start;
    ...
    if (!this.options.readonly &&
     ...
     this.options.deletable(calEvent,$calEvent)) {
      ...
      _selected24HourForIndex: function(index) {
        var options = this.options,
            timeStart = options.selectedTime.start,
            timeMid = options.selectedTime.mid,
            timeEnd = options.selectedTime.end;
          if (index === 0) {
            if (timeStart < 10){
              return '0' + timeStart + '<div class="sm">.00</div>';
            }
            else{
              return timeStart + '<div class="sm">.00</div>'; 
            }
          } else if (index === 1) {
            if (timeMid < 10){
              return '0' + timeMid + '<div class="sm">.00</div>';
            }
            else{
              return timeMid + '<div class="sm">.00</div>'; 
            }
          } else {
            if (timeEnd < 10){
              return '0' + timeEnd + '<div class="sm">.00</div>';
            }
            else{
              return timeEnd + '<div class="sm">.00</div>'; 
            }
          }
      },

      _selectedHourForIndex: function(index) {
        var options = this.options,
            timeStart = options.selectedTime.start,
            timeMid = options.selectedTime.mid,
            timeEnd = options.selectedTime.end;
          if (index === 0) { //midnight
            return options.selectedTime.start;
          } else if (index === 1) {
            return options.selectedTime.mid;
          } else {
            return options.selectedTime.end;
          }
      },
      ...
      _getHeaderDate: function(date) {
        ...
        var dayNameWrap = "<div class=\"day__name\">" + dayName + "</div>";
        return dayNameWrap + this._formatDate(date, 'd');
